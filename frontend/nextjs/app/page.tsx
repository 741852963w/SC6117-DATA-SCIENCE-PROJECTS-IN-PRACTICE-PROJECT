...
export default function Home() {
  ...
  const [bizCompetitors, setBizCompetitors] = useState("");
  ...
  const handleDisplayResult = async (newQuestion: string) => {
    ...
    if (chatBoxSettings.report_type === 'multi_agents' && langgraphHostUrl) {
      ...
      initializeWebSocket(newQuestion, chatBoxSettings, {
        business_region: bizRegion,
        business_customer: bizCustomer,
        business_competitors: bizCompetitors
      });
    }
  };
            ...
            <>
              ...
              {MINIMAL_UI && (
                <div className="mx-auto max-w-2xl px-4 py-12">
                  <h1 className="text-2xl font-semibold mb-4">VentureProof</h1>
                  <p className="text-sm text-gray-400 mb-6">Enter your venture idea and we will research and generate a structured report.</p>
                  <div className="space-y-3">
                    <div id="input-area" className="flex gap-2">
                      <input
                        className="flex-1 bg-black border border-gray-700 rounded px-3 py-2 text-gray-200 outline-none"
                        placeholder="e.g., Evaluate AI note‑taking SaaS for university students"
                        value={promptValue}
                        onChange={(e) => setPromptValue(e.target.value)}
                      />
                      <button
                        className="px-4 py-2 bg-gray-800 text-gray-100 rounded border border-gray-700"
                        onClick={() => handleDisplayResult(promptValue)}
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <input
                        className="bg-black border border-gray-700 rounded px-3 py-2 text-gray-200 outline-none"
                        placeholder="Target region (optional): e.g., China, North America, Southeast Asia"
                        value={bizRegion}
                        onChange={(e) => setBizRegion(e.target.value)}
                      />
                      <input
                        className="bg-black border border-gray-700 rounded px-3 py-2 text-gray-200 outline-none"
                        placeholder="Target customer (optional): e.g., SMBs, HR teams, developers"
                        value={bizCustomer}
                        onChange={(e) => setBizCustomer(e.target.value)}
                      />
                      <input
                        className="bg-black border border-gray-700 rounded px-3 py-2 text-gray-200 outline-none"
                        placeholder="Competitors (optional, comma‑separated): e.g., CompanyA, ProductB"
                        value={bizCompetitors}
                        onChange={(e) => setBizCompetitors(e.target.value)}
                      />
                    </div>
                  </div>
                </div>
